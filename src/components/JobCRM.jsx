...
import {
  ...
  AlignLeft, Shield, Settings, AlertTriangle, ChevronDown, ChevronUp, RefreshCw
} from 'lucide-react';
import {
...
const INITIAL_HEADER = {
  name: "Your Name", email: "email@example.com", location: "Metepec, Mexico", links: "github.com/yourusername • linkedin.com/in/you"
};
...
function FailedSourcesBadge({ failedSources }) {
  ...
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-2xl overflow-hidden mb-2 mx-[7.5%]">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-4 py-2.5 text-amber-800 hover:bg-amber-100 transition-colors">
        <div className="flex items-center gap-2 text-xs font-bold"><AlertTriangle size={14} className="text-amber-500" /> {failedSources.length} source{failedSources.length > 1 ? 's' : ''} unreachable this run</div>
      ...
      {open && (
        <div className="px-4 pb-3 space-y-1.5 border-t border-amber-100">
          ...
          <p className="text-[10px] text-amber-500 font-medium pt-1">These boards may have changed their API or the company slug may be wrong. Check server logs.</p>
        </div>
      )}
    </div>
  );
...
export default function JobCRM({ user, showToast, askConfirm }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [resSnap, bouncerSnap] = await Promise.all([getDoc(doc(db, RESUME_DOC(user.uid))), getDoc(doc(db, BOUNCER_DOC(user.uid)))]);
        if (resSnap.exists()) {
        ...
        if (bouncerSnap.exists()) setBouncer(bouncerSnap.data());
      } catch (e) {} finally { setIsLoadingBlocks(false); }
    })();
  }, [user]);

  // ── Live Scraping Effect ───────────────────────────────────────────────────
  useEffect(() => {
    if (!viewingJob || !viewingJob.url) { setScrapedDescription(""); return; }
    const scrape = async () => {
      setIsScraping(true);
      try {
        const res = await fetch(`/api/scrapeJob?url=${encodeURIComponent(viewingJob.url)}`);
        const data = await res.json();
        setScrapedDescription(data.description || "Could not automatically extract. Tap 'Open Job Post' below.");
      } catch (e) { setScrapedDescription("Failed to load description."); }
      setIsScraping(false);
    };
    scrape();
  }, [viewingJob]);
  ...
  const fetchLeads = useCallback(async (pageNum = 1) => {
    ...
    try {
      const res = await fetch('/api/fetchJobs', {
        ...
        body: JSON.stringify({ days: fetchDays, page: pageNum, vip: bouncer.vip, banned: bouncer.banned, locations: bouncer.locations })
      });
      const data = await res.json();
      ...
      if (data.jobs && data.jobs.length > 0) {
        ...
        if (newJobs.length === 0) { showToast(isFirstPage ? "No new leads — you're all caught up!" : 'All jobs on this page are already in your pipeline.', 'success'); return; }
        ...
        await batch.commit();
        showToast(`${isFirstPage ? 'Fetched' : 'Loaded'} ${newJobs.length} new lead${newJobs.length > 1 ? 's' : ''}!`, 'success');
      } else {
        showToast('No leads found for this page.', 'success');
      }
    } catch (e) { showToast('Network error fetching leads.', 'error'); } 
    finally { if (isFirstPage) setIsFetching(false); else setIsLoadingMore(false); }
  ...
  const handleSaveJob = async (e) => {
    ...
    try {
      if (editingId) { await updateDoc(doc(db, `users/${user.uid}/jobs`, editingId), { ...form, updatedAt: Timestamp.now() }); showToast("Job updated!", "success"); } 
      else { await addDoc(collection(db, `users/${user.uid}/jobs`), { ...form, createdAt: Timestamp.now(), updatedAt: Timestamp.now() }); showToast("Job added to pipeline!", "success"); }
      closeModal();
  ...
  const handleSaveBouncer = async () => {
    ...
    try { await setDoc(doc(db, BOUNCER_DOC(user.uid)), bouncer); showToast("Bouncer rules updated!", "success"); } 
    catch (e) { showToast("Error saving rules.", "error"); }
    setIsSavingBouncer(false);
  };

  // ── Helpers ────────────────────────────────────────────────────────────────
  const moveJob = async (id, jobStageId) => { const currentIdx = STAGES.findIndex(s => s.id === jobStageId); if (currentIdx < 0 || currentIdx >= STAGES.length - 1) return; await updateDoc(doc(db, `users/${user.uid}/jobs`, id), { stage: STAGES[currentIdx + 1].id, updatedAt: Timestamp.now() }); setViewingJob(null); };
  const fastDiscardLead = async (id) => { await deleteDoc(doc(db, `users/${user.uid}/jobs`, id)); setViewingJob(null); };
  const deleteJob = (id) => { askConfirm("Delete Job", "Remove this application from your pipeline?", async () => { await deleteDoc(doc(db, `users/${user.uid}/jobs`, id)); showToast("Job deleted.", "success"); setViewingJob(null); }); };
      ...
      <div className="flex gap-4 overflow-x-auto snap-x scrollbar-hide pb-2 shrink-0 w-full no-print">
        ...
        <div onClick={() => setActiveView('kanban')} className={`w-[85%] snap-center snap-always shrink-0 border p-6 rounded-[28px] shadow-sm flex flex-col justify-between transition-colors cursor-pointer ${activeView === 'kanban' ? 'bg-white border-indigo-200' : 'bg-slate-50/50 border-slate-200 opacity-60 grayscale'}`}>
          ...
          <div className="flex gap-2">
            <button onClick={(e) => { e.stopPropagation(); fetchLeads(1); }} disabled={isFetching} className="flex-1 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-xl transition-colors active:scale-95 shadow-sm disabled:opacity-50 flex justify-center items-center">
        ...
        <>
                ...
                <div key={stage.id} className={`w-[85%] shrink-0 snap-center snap-always bg-slate-50/50 border ${stage.border} rounded-[28px] flex flex-col max-h-[65vh]`}>
                  ...
                  <div className="p-3 overflow-y-auto space-y-3 flex-1 scrollbar-hide">
                    ...
                    {!isLoadingJobs && stageJobs.map(job => {
                      const cardContent = (
                        <>
                          <div className="flex justify-between items-start mb-2"><h3 className="font-bold text-slate-900 flex items-center gap-2">{job.company}</h3><button onClick={(e) => { e.stopPropagation(); openEdit(job); }} className="text-slate-300 hover:text-indigo-600 p-1"><Pencil size={14} /></button></div>
          ...
          {pagination?.hasNextPage && (
            <div className="flex flex-col items-center gap-1.5 pt-1 pb-4">
              <button onClick={() => fetchLeads(pagination.page + 1)} disabled={isLoadingMore} className="flex items-center gap-2 px-6 py-3 bg-white border border-indigo-200 text-indigo-600 font-bold text-sm rounded-2xl shadow-sm hover:bg-indigo-50 active:scale-95 transition-all disabled:opacity-50">
                {isLoadingMore ? <><Loader2 size={16} className="animate-spin" /> Loading page {pagination.page + 1}…</> : <><RefreshCw size={16} /> Load More <span className="font-normal text-indigo-400 text-xs">({pagination.totalMatches - pagination.page * pagination.pageSize} remaining)</span></>}
              </button>
              <p className="text-[10px] font-medium text-slate-400">Page {pagination.page} of {pagination.totalPages} · {pagination.totalMatches} matches</p>
            </div>
          )}
        </>
      )}
      ...
      {activeView === 'bouncer' && (
        <div className="flex-1 overflow-y-auto space-y-6 no-print scrollbar-hide pb-12 animate-in fade-in slide-in-from-bottom-4 px-1">
          <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 space-y-6">
            <div><label className="text-xs font-black uppercase tracking-widest text-emerald-600 ml-1 flex items-center gap-2 mb-2"><CheckCircle size={14}/> VIP Keywords (Include)</label><textarea rows="4" value={bouncer.vip} onChange={(e) => setBouncer({...bouncer, vip: e.target.value})} className="w-full bg-white border border-emerald-100 rounded-2xl px-4 py-3 text-sm text-slate-800 font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 resize-none leading-relaxed" /><p className="text-[10px] font-medium text-slate-400 ml-1 mt-1">Comma-separated. A job MUST have one of these in the title to enter your inbox.</p></div>
            <div><label className="text-xs font-black uppercase tracking-widest text-rose-600 ml-1 flex items-center gap-2 mb-2"><X size={14}/> Banned Keywords (Reject)</label><textarea rows="3" value={bouncer.banned} onChange={(e) => setBouncer({...bouncer, banned: e.target.value})} className="w-full bg-white border border-rose-100 rounded-2xl px-4 py-3 text-sm text-slate-800 font-medium outline-none focus:ring-2 focus:ring-rose-500/20 resize-none leading-relaxed" /><p className="text-[10px] font-medium text-slate-400 ml-1 mt-1">Jobs with these words in the title will be instantly deleted.</p></div>
            <div><label className="text-xs font-black uppercase tracking-widest text-slate-600 ml-1 flex items-center gap-2 mb-2"><Globe size={14}/> Banned Locations</label><textarea rows="2" value={bouncer.locations} onChange={(e) => setBouncer({...bouncer, locations: e.target.value})} className="w-full bg-white border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-800 font-medium outline-none focus:ring-2 focus:ring-slate-500/20 resize-none leading-relaxed" /></div>
            <Button onClick={handleSaveBouncer} disabled={isSavingBouncer} className="w-full !bg-slate-900 !mt-2">{isSavingBouncer ? <Loader2 size={18} className="animate-spin mx-auto"/> : "Save Filtering Rules"}</Button>
          </div>
        </div>
      )}
        ...
        {viewingJob && (
          <div className="space-y-6 px-1">
            ...
            <div className="flex flex-wrap gap-2">
              <div className="px-3 py-1.5 bg-slate-50 text-slate-600 font-bold text-sm rounded-xl border border-slate-200 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-slate-400"></span> {STAGES.find(s => s.id === viewingJob.stage)?.label || "Saved"}</div>
            ...
            {/* The Live Scraping Pane */}
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 max-h-[40vh] overflow-y-auto">
              <div className="flex items-center gap-2 text-slate-700 font-bold text-xs uppercase tracking-wider mb-3"><AlignLeft size={14} /> Full Description</div>
              {isScraping ? (
                <div className="flex flex-col items-center justify-center py-8 text-slate-400 gap-3"><Loader2 size={24} className="animate-spin text-indigo-400" /><span className="text-xs font-medium">Extracting from ATS...</span></div>
              ) : (
                <p className="text-slate-600 text-sm whitespace-pre-wrap leading-relaxed">{scrapedDescription || viewingJob.notes || "No description available."}</p>
              )}
            </div>

            <div className="pt-2 flex gap-3">
              {viewingJob.url && <a href={viewingJob.url} target="_blank" rel="noreferrer" className="flex-1 bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors">Open Site <ExternalLink size={16}/></a>}
      ...
      {blockForm && (
        <Modal isOpen={!!blockForm} onClose={() => setBlockForm(null)} title={blockForm.id ? "Edit Block" : "New Block"}>
          <form onSubmit={saveBlockForm} className="space-y-4 max-h-[70vh] overflow-y-auto p-1">
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-sm font-bold text-slate-600 ml-1">Category</label><select value={blockForm.type} onChange={(e) => setBlockForm({...blockForm, type: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 font-bold outline-none">{['Experience', 'Project', 'Education', 'Skills', 'Certification'].map(t => <option key={t} value={t}>{t}</option>)}</select></div>
              <div><label className="text-sm font-bold text-slate-600 ml-1">Date / Duration</label><input type="text" placeholder="e.g. 2024 - Present" value={blockForm.date} onChange={(e) => setBlockForm({...blockForm, date: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 font-medium outline-none" /></div>
            </div>
            <div><label className="text-sm font-bold text-slate-600 ml-1">Title & Company</label><input type="text" placeholder="e.g. Instructional Designer | Acme Corp" value={blockForm.title} onChange={(e) => setBlockForm({...blockForm, title: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 font-bold outline-none" required /></div>
            <div>
              <div className="flex justify-between items-center mb-2 ml-1"><label className="text-sm font-bold text-slate-600">Bullet Points</label><button type="button" onClick={() => setBlockForm({...blockForm, bullets: [...(blockForm.bullets||[]), '']})} className="text-xs font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 px-2 py-1 rounded-lg">+ Add</button></div>
              <div className="space-y-2">{(blockForm.bullets || []).map((bullet, i) => (<div key={i} className="flex gap-2"><textarea rows="2" value={bullet} onChange={(e) => { const newB = [...blockForm.bullets]; newB[i] = e.target.value; setBlockForm({...blockForm, bullets: newB}); }} className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 font-medium outline-none resize-none" /><button type="button" onClick={() => { const newB = blockForm.bullets.filter((_, idx) => idx !== i); setBlockForm({...blockForm, bullets: newB}); }} className="p-2 text-slate-400 hover:bg-red-50 hover:text-red-500 rounded-xl"><X size={16}/></button></div>))}</div>
            </div>
